...
class Example(object):
  ...
  def __init__(self, article, abstract_sentences, topic, vocab, hps):
    ...
    if len(topic_words) > hps.topic_max_enc_steps:
      topic_words = topic_words[:hps.topic_max_enc_steps]
    self.enc_len = len(article_words) # store the length after truncation but before padding
    self.topic_enc_len = len(topic_words)
    self.enc_input = [vocab.word2id(w) for w in article_words] # list of word ids; OOVs are represented by the id for UNK token
    self.topic_enc_input = [vocab.word2id(w) for w in topic_words]
  ...
  def pad_topic_encoder_input(self, topic_max_len, pad_id):
    """Pad the encoder input sequence with pad_id up to max_len."""
    while len(self.topic_enc_input) < topic_max_len:
      self.topic_enc_input.append(pad_id)
...
class Batch(object):
  ...
  def init_encoder_seq(self, example_list, hps):
    ...
    topic_max_enc_seq_len = max([ex.topic_enc_len for ex in example_list])

    # Pad the encoder input sequences up to the length of the longest sequence
    for ex in example_list:
      ex.pad_encoder_input(max_enc_seq_len, self.pad_id)
      ex.pad_topic_encoder_input(topic_max_enc_seq_len, self.pad_id)
    ...
    #self.topic_batch = np.zeros((hps.batch_size, max_enc_seq_len), dtype=np.int32)
    self.topic_batch = np.zeros((hps.batch_size, topic_max_enc_seq_len), dtype=np.int32)
    self.topic_enc_lens = np.zeros((hps.batch_size), dtype=np.int32)
    self.topic_enc_padding_mask = np.zeros((hps.batch_size, topic_max_enc_seq_len), dtype=np.float32)

    # Fill in the numpy arrays
    for i, ex in enumerate(example_list):
      self.enc_batch[i, :] = ex.enc_input[:]
      self.topic_batch[i, :] = ex.topic_enc_input[:]
      self.enc_lens[i] = ex.enc_len
      self.topic_enc_lens[i] = ex.topic_enc_len
      for j in xrange(ex.enc_len):
        self.enc_padding_mask[i][j] = 1
      for j in xrange(ex.topic_enc_len):
        self.topic_enc_padding_mask[i][j] = 1
  ...
  def store_orig_strings(self, example_list):
    ...
    self.original_topics = [ex.original_topic for ex in example_list]
...
class Batcher(object):
  ...
  def fill_example_queue(self):
    ...
    while True:
      try:
        (article, abstract, topic) = input_gen.next() # read the next example from file. article and abstract are both strings.
      ...
      example = Example(article, abstract_sentences, topic, self._vocab, self._hps) # Process into an Example.
  ...
  def text_generator(self, example_generator):
    ...
    while True:
      ...
      try:
        ...
        topic_text = e.features.feature['topic'].bytes_list.value[0]
      except ValueError:
