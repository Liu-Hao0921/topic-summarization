...
class BeamSearchDecoder(object):
  ...
  def get_score(self,original_topic,decode_word):
    count = 0
    topics = original_topic.split()
    for topic in topics:
      if topic in decode_word:
        count = count + 1
    return count
    #return (float)(count) / len(decode_word)
  def removes(self,decode_words):
    try:
      fst_stop_idx = decode_words.index(data.STOP_DECODING) # index of the (first) [STOP] symbol
      decode_words = decode_words[:fst_stop_idx]
    except ValueError:
  ...
  def decode(self):
    ...
    while True:
      ...
      standard_words = data.outputids2words(put_ids, self._vocab, (batch.art_oovs[0] if FLAGS.pointer_gen else None))
      # Extract the output ids from the hypothesis and convert back to words
      score = []
      for best_hyp in best_hyps:
        output_ids = [int(t) for t in best_hyp.tokens[1:]]
        tmp_decoded = data.outputids2words(output_ids, self._vocab, (batch.art_oovs[0] if FLAGS.pointer_gen else None))
      ...
      decoded_words = data.outputids2words(put_ids, self._vocab, (batch.art_oovs[0] if FLAGS.pointer_gen else None))
      ...
      if FLAGS.single_pass:
        ...
        self.write_for_attnvis(article_withunks, abstract_withunks, decoded_words, best_hyp.attn_dists, best_hyp.topic_attn_dists, best_hyp.p_gens) # write info to .json file for visualization tool
  ...
  def write_for_attnvis(self, article, abstract, decoded_words, attn_dists, topic_attn_dists, p_gens):
    ...
    to_write = {
        ...
        'topic_attn_dists': topic_attn_dists
    }
